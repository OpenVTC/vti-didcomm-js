 ...
 *  CARGO_TARGET_DIR env var, otherwise it's the helper crate's own
 *  `tools/roundtrip-helper/target/`.
 */
function helperPath() {
  const targetDir =
    process.env.CARGO_TARGET_DIR ||
    resolve(HERE, "..", "tools", "roundtrip-helper", "target");
...
if (!HELPER_AVAILABLE) {
  ...
  // ─── P-256 ────────────────────────────────────────────────────────────
  //
  // The same byte-for-byte proof as above, but on the NIST P-256
  // curve: the recipient private is the 32-byte scalar; the sender
  // public is the 65-byte uncompressed SEC1 point. If the Rust crate
  // unpacks our P-256 JWE, our EC `epk`, the X-coordinate `Z`, and the
  // Concat KDF inputs all match `affinidi-messaging-didcomm`.

  test("JS pack → Rust unpack: P-256 authcrypt", async () => {
    const sender = p256.generateKeyPair();
    const recipient = p256.generateKeyPair();
    const senderKid = "did:key:zSenderP256#p256-1";
    const recipientKid = "did:key:zRecipientP256#p256-1";

    const message = {
      id: "msg-p256-roundtrip",
      type: "https://example.com/test/1.0",
      from: senderKid.split("#")[0],
      to: [recipientKid.split("#")[0]],
      body: { hello: "p256-rust" },
    };
    ...
    const jwe = await pack({
      ...
      sender: {
        kid: senderKid,
        privateJwk: jwk.privateJwk("P-256", sender.privateKey, sender.publicKey, senderKid),
      },
      recipient: {
        kid: recipientKid,
        publicJwk: jwk.publicJwk("P-256", recipient.publicKey, recipientKid),
      },
    });
  ...
  test("JS pack → Rust unpack: P-256 anoncrypt", async () => {
    const recipient = p256.generateKeyPair();
    const recipientKid = "did:key:zRecipientP256#p256-1";

    const message = { id: "m", type: "t", body: { secret: "anon-p256-rust" } };
    ...
    assert.equal(resp.ok, true, `Rust unpack failed: ${JSON.stringify(resp)}`);
    assert.equal(resp.plaintext.body.secret, "anon-p256-rust");
  });
}
