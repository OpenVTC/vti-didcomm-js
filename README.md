...
Part of the wider **Verifiable Trust Community** project at
[github.com/OpenVTC](https://github.com/OpenVTC/), which builds open
...
- **Authcrypt** pack/unpack — ECDH-1PU + A256KW + **A256CBC-HS512**
  ...
  Key agreement over **X25519 or P-256** (curve taken from the
  recipient's key).
- **Anoncrypt** pack/unpack — ECDH-ES + A256KW + A256CBC-HS512. No
  sender identity; used for the `routing/2.0/forward` envelope to a
  mediator. X25519 or P-256.
- **DID resolution** — `did:key` (Ed25519/X25519/P-256/secp256k1,
  in-tree), `did:peer` (numalgo 2, in-tree) and `did:webvh` (via
  [`didwebvh-ts`], full hash-chain + Data-Integrity verification).
  Pluggable dispatcher for adding methods.
