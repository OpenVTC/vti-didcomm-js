...
export async function packAnoncrypt({ message, recipient }) {
  ...
  if (!keyAgreement.isSupported(crv)) {
    throw new Error(`packAnoncrypt: recipient key curve ${JSON.stringify(crv)} is not a supported key-agreement curve`);
  }
  const recipientPub = jwk.rawPublic(recipient.publicJwk);

  // Ephemeral keypair on the recipient's curve + CEK/IV (64-byte
  // CEK, 16-byte CBC IV).
