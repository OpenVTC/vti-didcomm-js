...
export async function unpack(jweJson, recipient, sender) {
  ...
  // 4. Derive the KEK (algorithm-dependent). The recipient's private
  //    key must be on the same curve as the ephemeral key (`epk`).
  if (jwk.curveOf(recipient.privateJwk) !== crv) {
    throw new Error(
      `unpack: recipient key curve (${jwk.curveOf(recipient.privateJwk)}) does not match epk curve (${crv})`,
    );
  }
  const recipientPriv = jwk.rawPrivate(recipient.privateJwk);
  const ephemeralPublic = jwk.rawPublic(header.epk);
  ...
  if (isAuthcrypt) {
    ...
    if (jwk.curveOf(sender.publicJwk) !== crv) {
      throw new Error(
...
/**
 * Validate the JWE `epk` (ephemeral public key) and return its
 * key-agreement curve. We accept the two curves DIDComm key agreement
 * uses: OKP/X25519 and EC/P-256.
 *
 * @param {Object} epk
 * @returns {"X25519"|"P-256"}
 */
function epkCurve(epk) {
  if (!epk || typeof epk !== "object") {
    throw new Error("unpack: protected header missing epk");
  }
  if (epk.kty === "OKP" && epk.crv === "X25519") return "X25519";
  if (epk.kty === "EC" && epk.crv === "P-256") return "P-256";
  throw new Error(
    `unpack: unsupported epk ${JSON.stringify({ kty: epk.kty, crv: epk.crv })}; expected OKP/X25519 or EC/P-256`,
  );
}
