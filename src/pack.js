...
//
//   1. Generate ephemeral keypair on the recipient's curve
//      (X25519 or P-256).
...
import * as keyAgreement from "./key-agreement.js";
...
export async function pack({ message, sender, recipient }) {
  ...
  // ECDH-1PU requires the ephemeral, sender, and recipient keys to
  // share one curve. Derive it from the recipient's key-agreement key
  // and require the sender's static key to match.
  const crv = jwk.curveOf(recipient.publicJwk);
  if (!keyAgreement.isSupported(crv)) {
    throw new Error(`pack: recipient key curve ${JSON.stringify(crv)} is not a supported key-agreement curve`);
  }
  if (jwk.curveOf(sender.privateJwk) !== crv) {
    throw new Error(
      `pack: sender key curve (${jwk.curveOf(sender.privateJwk)}) must match recipient curve (${crv}) for ECDH-1PU`,
    );
  }
  const senderPriv = jwk.rawPrivate(sender.privateJwk);
  const recipientPub = jwk.rawPublic(recipient.publicJwk);

  // 1. Ephemeral keypair on the recipient's curve.
